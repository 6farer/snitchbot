...
module.exports = (robot)->
  robot.respond /9gag( me)?/i, (message)->
    get_price message, false, (text)->
      message.send text

get_price = (message, location, response_handler)->
  domain = "http://crypsty.com"

  message.http( domain ).get() (error, response, body)->
    return response_handler "Sorry, something went wrong" if error

    if response.statusCode == 302
      location = response.headers['location']
      return get_price( message, location, response_handler )

    img_src = get_meme_image( body, "#btc-markets ul li" )
